...
const float BASELINE_INCREASE_RATE = 0.002;  // Tăng chậm (0.2% mỗi lần)
const float BASELINE_DECREASE_RATE = 0.05;   // Giảm nhanh (5% mỗi lần)
const unsigned long BASELINE_UPDATE_INTERVAL = 100; // ms
const unsigned long EVENT_FREEZE_TIME = 3000; // 3 giây freeze khi có event liên tục
const float EVENT_THRESHOLD_RATIO = 1.15; // Event = 15% trên baseline
const float REFERENCE_PRESSURE = 0.00002; // 20 µPa cho tính dB
...
// Hàm cập nhật baseline với các nguyên lý:
// 1. Short-term và long-term baseline
// 2. Tăng chậm, giảm nhanh
// 3. Freeze khi có event liên tục
// 4. Min filter
void updateBaseline(int mic_index) {
  unsigned long current_time = millis();
  
  // Kiểm tra khoảng thời gian cập nhật
  ...
  if (current_rms < 0.001) return; // Tránh giá trị quá nhỏ
  
  // === MIN FILTER ===
  // Lưu giá trị RMS vào buffer và tìm giá trị nhỏ nhất
  ...
  // === PHÁT HIỆN EVENT ===
  // Event = RMS vượt quá 15% so với long-term baseline
  ...
  // === FREEZE BASELINE NẾU CÓ EVENT LIÊN TỤC ===
  // Không cập nhật baseline trong 3 giây đầu của event
  if (in_event[mic_index] && (current_time - event_start_time[mic_index] < EVENT_FREEZE_TIME)) {
    return; // Freeze - không cập nhật baseline
  }
  ...
  // Sử dụng min filter để tránh nhiễu tức thời
...
void OnDataRecv(const esp_now_recv_info *info, const uint8_t *incomingData, int len) {
  ...
  if (slave_id >= 1 && slave_id <= 4) {
    ...
    // === TÍNH RMS ===
    // Lưu bình phương của giá trị vào buffer
    ...
    // Tính trung bình và lấy căn bậc 2
    float sum = 0;
    ...
    if (rms_values[index] > REFERENCE_PRESSURE) {
      db_values[index] = 20.0 * log10(rms_values[index] / REFERENCE_PRESSURE);
    } else {
      db_values[index] = 0.0;
    }
...
<body>
  <div class="container">
    ...
    <div class="card">
      ...
      <p style="margin: 10px 0;">🔴 = Event đang xảy ra (RMS > 115% baseline)</p>
      <p style="margin: 10px 0;">🟢 = Không có event</p>
      <p style="margin: 10px 0;">Baseline tăng chậm (0.2%/lần), giảm nhanh (5%/lần)</p>
      <p style="margin: 10px 0;">Freeze baseline trong 3 giây đầu khi có event liên tục</p>
    </div>
  </div>
      ...
      fetch('/data')
        ...
        .then(data => {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            if (data.rms[i] > data.baseline_long[i] * 1.15) {
...
for (int i = 0; i < NUM_ZONES; i++) {
  ...
  if (max_influence == 0.0) {
    P_est_zones[i] = 0.0;
      ...
      // Tính giá trị ước lượng cho vùng
      if (mic_co_tin_hieu == 0 || tong_trong_so == 0.0) {
        P_est_zones[i] = 0.0;
      } else {
