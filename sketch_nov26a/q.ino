...
// --- NGƯỠNG KÍCH HOẠT HỆ THỐNG (MỚI) ---
float DB_KICH_HOAT = 1000.0;  // Ngưỡng dB tổng để kích hoạt hệ thống
bool he_thong_active = false;  // Trạng thái hệ thống
bool da_reset = false;         // Đánh dấu đã reset khi vừa vượt ngưỡng
...
// Hàm reset toàn bộ giá trị tính toán
void reset_gia_tri_tinh_toan() {
  for (int i = 0; i < NUM_ZONES; i++) {
    P_est_zones[i] = 0.0;
  }
  strip_Vung.clear();
  strip_Vung.show();
  Serial.println(">> ĐÃ RESET GIÁ TRỊ TÍNH TOÁN <<");
}
...
<body>
  <div class="container">
    ...
    <div class="db-total" id="dbTotal">dB Tổng: 0</div>
    ...
    <div class="card">
      ...
      <form id="settingsForm">
        <div class="setting-row" style="background: rgba(255,215,0,0.2); border: 2px solid #f39c12;">
          <label><strong>🔊 Ngưỡng dB Kích Hoạt:</strong></label>
          <input type="number" id="dbActivate" step="10" value="1000">
          <button type="button" class="btn" style="padding: 5px 15px; background: #f39c12;" onclick="updateDbActivate()">Cập nhật</button>
        </div>
  ...
  <script>
    function updateData() {
      fetch('/data')
        ...
        .then(data => {
          ...
          document.getElementById('dbTotal').textContent = 'dB Tổng: ' + data.dbTotal.toFixed(0);
          
          let modeText = ['MODE 0: TẮT', 'MODE 1: Bình thường', 'MODE 2: Yên tĩnh'];
          let statusEl = document.getElementById('status');
          
          if(data.active) {
            statusEl.textContent = '🟢 HOẠT ĐỘNG - ' + modeText[data.mode];
            statusEl.className = 'status active';
          } else {
            statusEl.textContent = '🔴 CHỜ KÍCH HOẠT - ' + modeText[data.mode];
    ...
    function updateDbActivate() {
      let val = document.getElementById('dbActivate').value;
      fetch('/setDbActivate?value=' + val).then(() => alert('Đã cập nhật ngưỡng kích hoạt!'));
    }
...
void handleData() {
  ...
  json += "\"dbTotal\":" + String(db_total) + ",";
  json += "\"active\":" + String(he_thong_active ? "true" : "false");
  json += "}";
...
void handleSetDbActivate() {
  if (server.hasArg("value")) {
    DB_KICH_HOAT = server.arg("value").toFloat();
  }
  server.send(200, "text/plain", "OK");
}
...
void setup() {
  Serial.begin(115200);
  Serial.println("ESP32 Noise Monitor with DB Activation Threshold");
  ...
  server.on("/setDbActivate", handleSetDbActivate);
...
void loop() {
  ...
  if (buttonPressed) {
    ...
    if (count == 0) {
      ...
      he_thong_active = false;
      da_reset = false;
      reset_gia_tri_tinh_toan();
      strip_Mode.setPixelColor(0, strip_Mode.Color(100, 100, 100));
      strip_Mode.show();
    } else if (count == 1) {
  ...
  if (count == 0) {
    ...
    // Tính tổng dB từ tất cả microphone
    float db_total = 0;
    for(int i = 0; i < NUM_MICS; i++) {
      db_total += P_muot_values[i];
    }
    ...
    if (db_total > DB_KICH_HOAT) {
      // Vừa vượt ngưỡng lần đầu -> Reset
      if (!he_thong_active) {
        he_thong_active = true;
        reset_gia_tri_tinh_toan();
        Serial.println(">> HỆ THỐNG ĐƯỢC KÍCH HOẠT - BẮT ĐẦU TÍNH TOÁN <<");
        Serial.print("dB Tổng: ");
        Serial.println(db_total);
      }
      
      // Tính IDW khi hệ thống đã active
      for (int i = 0; i < NUM_ZONES; i++) {
      ...
      // Dưới ngưỡng -> Tắt hệ thống
      if (he_thong_active) {
        he_thong_active = false;
        reset_gia_tri_tinh_toan();
        Serial.println(">> HỆ THỐNG TẮT - dB DƯỚI NGƯỠNG <<");
      }
    }
  }
