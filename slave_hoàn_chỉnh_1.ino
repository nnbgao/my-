 ...
 *
 * 🔧 Cập nhật:
 * - Sửa chữ ký hàm OnDataSent cho đúng.
 * - Thêm 'volatile' vào các biến hiệu chỉnh (fix lỗi logic).
 * - Xóa include <esp_wifi.h> (đã có trong WiFi.h).
 * - Thêm kiểm tra race-condition trong loop().
 */
...
// ✨ ĐÃ SỬA: Dùng đúng chữ ký hàm cho esp_now_register_send_cb
...
void loop() {
  ...
  // ✨ ĐÃ SỬA: Thêm kiểm tra race condition
  // Đảm bảo không gửi nếu lệnh ngắt vừa set 'is_calibrating'
  if (is_calibrating) {
    return;
  }
